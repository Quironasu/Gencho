...
async def sendLog(interaction, account, service, premium):
  ...
  embed = discord.Embed()
  embed.set_author(name=author_name + ' - ' + str(interaction.user.id),
...
def normal_cooldown(interaction: discord.Interaction):
  if not config[
      'admin-cooldown'] and interaction.user.guild_permissions.administrator:
    return None
  else:
...
@tree.command(name="gen",
              description="Generar una cuenta.",
...
async def gen(interaction: discord.Interaction, account: str):
  if (str(interaction.channel_id) != config["free-channel-id"]
  ...
  for file in glob.glob(os.path.join(directory, 'accounts/*.txt')):
    file_name = os.path.basename(file).split('.')[0]
    if file_name.lower() == account.lower():
      if os.stat(file).st_size == 0:
        await interaction.response.send_message("`" + file_name.upper() +
                                                "` esta agotado.")
        return
      with open(file, "r+", encoding="utf-8") as fp:
      ...
      await simpleEmbed(interaction, config["messages"]["account-generated"],
                        False)
      if config["logs-switch"]:
        await sendLog(interaction, acc_line, file_name, False)
      return
  await simpleEmbed(interaction, config["messages"]["service-doesnt-exist"],
                    True)
...
async def gencmd_error(interaction: discord.Interaction, error):
  if isinstance(error, app_commands.CommandOnCooldown):
    await interaction.response.send_message(
        f'Este comando está en cooldown. Por favor, inténtalo de nuevo en {error.retry_after:.2f} seconds.',
        ephemeral=True)
...
@tree.command(name="pgen",
              description="Generar una cuenta premium.",
              guild=discord.Object(id=config["guild-id"]))
async def pgen(interaction: discord.Interaction, account: str):
  if (str(interaction.channel_id) != config["premium-channel-id"]
      and config["channel-specific-switch"]):
    await simpleEmbed(interaction, "Este no es el canal adecuado!", True)
    return
  ...
  if (premium_role_id is not None
  ...
  channel = await interaction.user.create_dm()
  for file in glob.glob(os.path.join(directory, 'paccounts/*.txt')):
    file_name = os.path.basename(file).split('.')[0]
    if file_name.lower() == account.lower():
      if os.stat(file).st_size == 0:
        await interaction.response.send_message("`" + file_name.upper() +
...
async def gencmd_error(interaction: discord.Interaction, error):
  if isinstance(error, app_commands.MissingRole):
    await interaction.response.send_message(
        config["messages"]["no-permissions"], ephemeral=True)
...
@tree.command(name="create",
              description="Crea un nuevo servicio. (ADMIN ONLY)",
              guild=discord.Object(id=config["guild-id"]))
...
async def create(interaction: discord.Interaction, premium: bool, name: str):
  if premium:
    ...
    await interaction.response.send_message("Nuevo servicio creado: ``" +
                                            name.lower() + "``")
...
@tree.command(name="remove",
              description="Elimina un servicio. (ADMIN ONLY)",
              guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def remove(interaction: discord.Interaction, premium: bool, name: str):
  ...
  if premium:
    if os.path.exists(directory + '/paccounts'):
      ...
      if os.path.exists(premium_log_file):
        ...
        await interaction.response.send_message(
            f'El servicio "{service_name}" ha sido eliminado.')
        return
  ...
  await interaction.response.send_message(
      f'No se encontró el servicio "{service_name}".')


@tree.command(
    name="restock",
    description="Restock cuentas para un servicio. (SOLO ADMINISTRADORES)",
    guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def restock(interaction: discord.Interaction, premium: bool,
                  service: str, accounts: str):
  if premium:
    for file in glob.glob(directory + '/paccounts/*.txt'):
        ...
        with open(file, 'a') as fp:
          for account in account_list:
            if not fp.tell() == 0:
              fp.write('\n')
            fp.write(account)
        await interaction.response.send_message("Operación exitosa.",
                                                ephemeral=True)
        return
  elif not premium:
    for file in glob.glob(directory + '/accounts/*.txt'):
...
@tree.command(
    name="setupchannelgen",
    description="Establecer canales para servicios premium y gratuitos.",
    guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def setup_channels(interaction: discord.Interaction, premium: bool,
  ...
  if premium:
    ...
    await interaction.response.send_message(
        "Canal premium configurado correctamente.")
  else:
    config["free-channel-id"] = channel_id
    await interaction.response.send_message(
        "Canal gratuito configurado exitosamente.")

  with open('config.json', 'w') as config_file:
    json.dump(config, config_file, indent=4)


@tree.command(name="setuppremiumrole",
              description="Establecer el rol premium.",
              guild=discord.Object(id=config["guild-id"]))
async def setup_role(interaction: discord.Interaction,
  ...
  try:
    ...
    with open('config.json', 'w') as config_file:
      json.dump(config, config_file, indent=4)
    await interaction.response.send_message(
        "El rol premium se estableció correctamente.")
  except Exception as e:
    print(f"Error al configurar el rol premium: {e}")


@tree.command(name="setupchannellog",
              description="Establecer el canal para logs.",
              guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def setup_logchannel(interaction: discord.Interaction,
                           channel: discord.TextChannel):
  channel_id = str(channel.id)

  config["logs-channel-id"] = channel_id
  await interaction.response.send_message(
      "Canal de logs configurado exitosamente.")
...
@tree.command(
    name="setpremiumcooldown",
    description="Habilitar o deshabilitar el cooldown para rol premium.",
    guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def set_premiumcooldown(interaction: discord.Interaction, enable: bool):
  ...
  if enable:
    await interaction.response.send_message(
        "Premium cooldown ha sido habilitado.")
  else:
    await interaction.response.send_message(
        "Premium cooldown ha sido deshabilitado.")


@tree.command(
    name="setadmincooldown",
    description="Habilitar o deshabilitar el cooldown para adminstrador.",
    guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def set_admincooldown(interaction: discord.Interaction, enable: bool):
  ...
  if enable:
    await interaction.response.send_message(
        "Admin cooldown ha sido habilitado.")
  else:
    await interaction.response.send_message(
        "Admin cooldown ha sido deshabilitado.")


@tree.command(name="setlogsswitch",
              description="Habilitar o deshabilitar el uso de logs.",
              guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
...
@tree.command(
    name="setexclusivechannels",
    description=
    "Habilitar o deshabilitar el uso de canales especificos para generar.",
    guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def set_specificchannels(interaction: discord.Interaction, enable: bool):
  ...
  if enable:
    await interaction.response.send_message(
        "Canales especificos han sido habilitados.")
  else:
    await interaction.response.send_message(
        "Canales especificos han sido deshabilitados.")
...
async def setup_channels_error(interaction: discord.Interaction, error):
  if isinstance(error, app_commands.MissingPermissions):
    ...
    await interaction.response.send_message(
        "Se produjo un error al procesar el comando.", ephemeral=True)


@tree.command(name="stock",
              description="Mostrar el stock.",
              guild=discord.Object(id=config["guild-id"]))
async def stock(interaction: discord.Interaction):
  ...
  if not len(account_list) <= 0:
    ...
    embed.add_field(name="Generador Gratis",
                    value='\n'.join(new_acc_list),
                    inline=False)
  ...
  if not len(account_list) <= 0:
    ...
    embed.add_field(name="Generador Premium",
                    value='\n'.join(new_acc_list),
                    inline=False)
